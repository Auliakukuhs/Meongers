<?php
	...
	if(!empty($_FILES["file"]["name"])) {
		...
		move_uploaded_file($_FILES["file"]["tmp_name"], "../../images/bukti/".$nama_file);

		$updategambar = ", bukti='$nama_file'";
	}
	...
	if($button == "Konfirmasi"){
		...
		$queryPembayaran = mysqli_query($koneksi, "INSERT INTO konfirmasi_pembayaran (pesanan_id, nomor_rekening, nama_account, tanggal_transfer, bukti)
		...
		if($queryPembayaran){
			mysqli_query($koneksi, "UPDATE pesanan SET status='1' WHERE pesanan_id='$pesanan_id',
																																	$updategambar WHERE barang_id='$pesanan_id'");
		}
